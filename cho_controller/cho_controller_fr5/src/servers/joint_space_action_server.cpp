...
rclcpp_action::GoalResponse FR5JointSpaceActionServer::handle_goal(
...
rclcpp_action::CancelResponse FR5JointSpaceActionServer::handle_cancel(
    const std::shared_ptr<JointSpaceGoalHandle> /*goal_handle*/)
{
    return rclcpp_action::CancelResponse::ACCEPT;
}

void FR5JointSpaceActionServer::handle_accepted(
    const std::shared_ptr<JointSpaceGoalHandle> goal_handle)
{
...
bool FR5JointSpaceActionServer::compute(const rclcpp::Time & current_time, FR5State & state)
{
    if (!control_running_ || !goal_handle_ || !goal_handle_->is_active()) {
        return false;
    }
    ...
    if (!initialized_) {
        // Start the cubic from the last commanded setpoint (q_ref), not the
        // measured, gravity-drooped position. Seeding from the drooped state
        // stepped the command down by the droop on the first cycle, producing a
        // visible dip-then-go lurch at the start of every goal. q_ref is the held
        // idle command (seeded from the measurement in on_activate and maintained
        // by clip_position), so the command stays continuous across idle -> motion.
        start_time_ = current_time;
        trajectory_->setStartTime(start_time_.seconds());
        trajectory_->setInitSample(state.q_ref);
        initialized_ = true;
    }
