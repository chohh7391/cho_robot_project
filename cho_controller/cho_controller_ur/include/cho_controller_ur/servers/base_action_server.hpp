...
public:
    ...
    URBaseActionServer(rclcpp_lifecycle::LifecycleNode::SharedPtr node, std::string action_name,
                       int num_dof)
    : node_(node), action_name_(action_name), num_dof_(num_dof) {}

    virtual ~URBaseActionServer() = default;

    virtual void init() {
        action_server_ = rclcpp_action::create_server<ActionT>(
            node_,
            action_name_,
            std::bind(&URBaseActionServer::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
            std::bind(&URBaseActionServer::handle_cancel, this, std::placeholders::_1),
            std::bind(&URBaseActionServer::handle_accepted, this, std::placeholders::_1)
        );
    ...
    virtual bool compute(const rclcpp::Time & current_time, URState & state) = 0;
