...
GEAR_MESH_FRANKA_DEFAULT_POSITION = make_joint_state(
...
GEAR_MESH_FRANKA_APPROACH_POSE = make_pose(
    position=[0.6 + 0.02025 + x_offset, 0.0 + y_offset, 0.05 + 0.025 + 0.035],
    orientation=approach_orientation
)


# ==========================================
# 🧠 Franka Gear Mesh (VLA) 트리 조립
# ==========================================
def create_franka_gear_mesh_tree(robot_config=None) -> py_trees.behaviour.Behaviour:
    mission_sequence = py_trees.composites.Sequence(name="Gear_Mesh_Sequence", memory=True)
