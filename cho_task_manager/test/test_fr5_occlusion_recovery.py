...
def test_the_return_is_slow_enough_for_the_cells_rate_ceiling():
    ...
    returns = [leaf for leaf in _leaves(_tree())
               if isinstance(leaf, JointSpaceActionBehavior)
...
def test_the_sweeps_search_wide_before_they_look_close():
    ...
    for leaf in _leaves(_tree()):
        if isinstance(leaf, OcclusionSweepBehavior):
            assert leaf.sweep.waypoints[0].name.startswith('survey')
            assert leaf.sweep.waypoints[-1].name.startswith('close')
            assert leaf.sweep.min_decision_margin > 0.0


def test_a_sweep_can_run_its_whole_raster_inside_its_own_ceiling():
    ...
    for leaf in _leaves(_tree()):
        if isinstance(leaf, OcclusionSweepBehavior):
            ...
            assert sweep.timeout_sec > worst, (sweep.object, worst, sweep.timeout_sec)


def test_the_latch_records_the_key_and_frame_a_motion_leaf_would_read():
    config = _config()
    latches = [leaf for leaf in _leaves(_tree())
               if isinstance(leaf, PoseTargetBehavior)]
...
def test_the_sweeps_drive_the_controller_the_rest_of_the_tree_uses():
