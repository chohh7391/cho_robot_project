...
Same three-part structure as the Franka Isaac bringup, and for the same reasons
...
of them):
  ...
  2. a controller_manager whose hardware plugin is
     ...
     ros2_control_node, which is upstream's node plus sim-clock pacing.
  3. the spawners, held back until Isaac announces itself, then the command gate.

The UR has no gripper here and is position-controlled only, so there is a single
...
def generate_launch_description():
    ur_desc = get_package_share_directory('cho_description_ur')
    bringup = get_package_share_directory('cho_bringup_ur')
    return LaunchDescription([
        ...
        DeclareLaunchArgument('ee_name', default_value='tool0'),
        DeclareLaunchArgument('bringup_type', default_value='isaac'),
        DeclareLaunchArgument('use_sim_time', default_value='true'),
        DeclareLaunchArgument(
            'urdf_file',
            default_value=os.path.join(ur_desc, 'urdf', 'ur5e.urdf'),
        ),
        DeclareLaunchArgument(
            'controllers_file',
        ...
        DeclareLaunchArgument(
            ...
            default_value=os.path.join(ur_desc, 'usd', 'ur5e', 'ur5e.usda'),
