...
def generate_launch_description():
    ...
    static_scene = Node(
        ...
        name='franka_static_planning_scene', output='screen', parameters=[{
    ...
    wait = Node(
        ...
        name='wait_for_franka_static_planning_scene', output='screen', parameters=[{
            'ready_service': metadata['ready_service'],
    ...
    def after_gate(event, context):
        if event.returncode != 0:
            return [EmitEvent(event=Shutdown(
                reason=f'Franka static planning scene gate failed ({event.returncode})'))]
